...
function AddEdit(props) {
    ...
    if (prod) {
        formOptions.defaultValues = props.prod;
    }
    ...
    const [selected, setSelected] = useState(prod ? new Set([prod.category_id]) : new Set([categories?.at(0)]));
    ...
    const [active, setStatus] = useState(prod ? prod.status : true);
    ...
    async function onSubmit(data) {
        ...
        try {
            ...
            let message;
            if (prod) {
                await catService.update(prod.id, formData);
                ...
                <div className="mb-3 col-3">
                    ...
                    <Dropdown>
                        <Dropdown.Button flat className='w-250' name='type' {...register('type')} value={selectedValue}>
                            {categories?.at(parseInt(selectedValue))?.name}
                        </Dropdown.Button>
                        <Dropdown.Menu 
